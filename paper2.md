...
At a glance, the corpus comprises 305 studies drawn from the three target journals, all published before the close of 2024. This comprehensive sweep, our first analytical waypoint, anchors the subsequent thematic and technological breakdowns.

3.1. Overall Research Volume  
To calibrate scale, we begin with sheer output. Across Presence, Frontiers in Virtual Reality, and Virtual Reality, we achieved 100 % coverage of XR articles, tallying 305 distinct investigations. Such reach confirms escalating scholarly momentum in virtual, augmented, and mixed reality. More importantly, it establishes a stable foundation for cross-study comparison.
...
Moving from description to interpretation, we synthesize the implications of the 305-study corpus. Collectively, the articles chart a trajectory of robust growth and methodical sophistication, a transition from niche experimentation to established practice in healthcare, education, and industry.
...
Turning to persistent obstacles, we identify three intertwined bottlenecks that slow XR's march toward ubiquity.

First, Cybersickness endures. Although only 10 studies, or 3.3 % of the corpus, investigate this malady, they document discomfort rates as high as 40 %, a figure incompatible with mass deployment. Current mitigation strategies cluster into physiological monitoring (Yalcin et al., 2024; Sameri et al., 2024), perceptual design tweaks such as avatar anchoring (Makani et al., 2024), and vection control (Teixeira et al., 2024). Yet, as several authors concede, no silver bullet exists. Until detection systems become predictive, not merely reactive, and until design guidelines translate across genres, cybersickness will remain a gatekeeper.

Second, interaction complexity grows. More sensors mean more friction unless interfaces remain intuitive. Contemporary Human–Computer Interaction studies converge on three levers—gesture recognition, haptic fidelity, and holistic UX modelling (Reski et al., 2024; Terenti et al., 2024; Gong et al., 2024). Front-loaded evaluations show that reducing cognitive load by even small margins (Zielke et al., 2024) yields measurable gains in task efficiency. However, fragmented evaluation protocols impede accumulation of evidence, suggesting an urgent need for shared benchmarks.

Third, longevity is under-tested. Most experiments last minutes, occasionally hours. Prolonged exposure—multi-day training cycles, month-long rehabilitation regimens—remains anecdotal. Future research should embed longitudinal designs, measure retention, and calibrate comfort over weeks, not sessions.
...
• Standardise usability benchmarks, allowing gesture, haptic, and voice interfaces to be compared across laboratories.  
...
• Embed accessibility from the outset, designing hardware, software, and content through participatory methods that centre diverse abilities.  
• Interlink social-platform studies with governance research, thereby translating millions of user-hours into actionable policy.

Pursuing these five trajectories will demand interdisciplinary consortia, open datasets, and replicable protocols. If the community embraces that agenda, the next review will chronicle not only incremental improvements but transformative shifts in how humans learn, heal, and collaborate through immersive media.
